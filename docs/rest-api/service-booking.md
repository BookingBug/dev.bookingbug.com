...
@startuml;
actor User;
participant "Enter Location" as A;
participant "List Stores" as B;
participant "List Services" as C;
participant "List Available Slots" as D;
participant "Collect User Details" as E;
participant "Confirmation" as F;
participant "Checkout" as G;
User -> A: Enter location;
activate A;
A -> B: GET <company-id>/addresses;
deactivate A;
activate B;
B -> User: Select store;
deactivate B;
User -> C: GET <company-id>/services;
activate C;
C -> User: Select service and datetime;
deactivate C;
User -> D: GET <company_id>/services?service_id=<time_data>;
activate D;
D -> User: Select avlaible slot;
deactivate D;
User -> E: GET <company-id>/questions?detail_group_id=<detail-group-id>;
activate E;
E -> User: Event questions;
User -> E: POST <company_id>/client;
deactivate E;
User -> F: POST <company-id>/basket/add_item;
activate F;
F -> User: User confirmation;
deactivate F;
User -> G: POST <company-id>/basket/checkout;
activate G;
